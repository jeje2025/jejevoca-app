...
export function Header({ profileImage, userXP, recentXPGain, showXPAnimation, onXPAnimationComplete, levelProgress, userName, onLogout }: HeaderProps) {
  ...
  return (
    <div className="flex items-center justify-between px-8 py-8">
      <div className="flex items-center gap-4">
        <div className="relative">{/* Admin Mode Hidden Trigger - Triple tap on logo */}
          <motion.button
            onClick={(e) => {
              const now = Date.now();
              const tapTimes = (window as any).__adminTapTimes || [];
              tapTimes.push(now);
              (window as any).__adminTapTimes = tapTimes.filter((t: number) => now - t < 1000);
              
              if ((window as any).__adminTapTimes.length >= 5) {
                (window as any).__adminTapTimes = [];
      ...
      <motion.button
        ...
        onClick={onLogout}
      >
        <LogOut className="w-5 h-5 text-[#091A7A] stroke-[1.5]" />
