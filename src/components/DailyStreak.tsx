...
export function DailyStreak({ streakCount }: DailyStreakProps) {
  ...
  const completedDays = Array.from({ length: 6 }, (_, i) => i < streakCount);
    ...
    >
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <span className="text-section-header text-[#091A7A]">주간 학습</span>
        </div>
        <div className="px-3 py-1.5 bg-gradient-to-r from-[#F59E0B]/20 to-[#F59E0B]/10 rounded-[50px] border border-[#F59E0B]/20">
          <span className="text-small font-medium text-[#F59E0B]">{streakCount}일 연속</span>
        </div>
      </div>
