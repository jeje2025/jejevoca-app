  ...
  try {
    ...
    const dbWords = await Promise.all(words.map(async (word, index) => {
      ...
      return {
        ...
        godlife_example: word.godlifeExample || '',
...
app.get('/make-server-c9fd9b61/words/:vol/:day', async (c) => {
  try {
    ...
    const formattedWords = words.map(w => ({
      ...
      example: w.example,
      englishExample: w.english_example,
      godlifeExample: w.godlife_example,
...
// Generate part of speech using Gemini API (admin only)
app.post('/make-server-c9fd9b61/words/generate-pos', async (c) => {
  try {
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
    const supabase = getSupabaseClient();
    ...
    // Helper function to get part of speech from Gemini API
    const getPartOfSpeech = async (wordText: string, meaning: string): Promise<string> => {
      if (!wordText || wordText.trim() === '') return '';
      ...
      try {
        const prompt = `Given the English word "${wordText}" with Korean meaning "${meaning}", determine its part of speech. Return ONLY one of these exact values: noun, verb, adjective, adverb, pronoun, preposition, conjunction, interjection, determiner. If it can be multiple, choose the most common usage.`;
        ...
        if (!response.ok) {
          console.error(`Gemini API error for ${wordText}:`, await response.text());
          return '';
        }

        const data = await response.json();
        const pos = data.candidates?.[0]?.content?.parts?.[0]?.text?.trim().toLowerCase() || '';

        // Validate the response is one of the expected parts of speech
        const validPos = ['noun', 'verb', 'adjective', 'adverb', 'pronoun', 'preposition', 'conjunction', 'interjection', 'determiner'];
        ...
        console.error(`Error fetching part of speech for ${wordText}:`, error);
        return '';
      }
    };
    ...
    if (allWords) {
      // Update all words without part of speech
      const { data: words, error: fetchError } = await supabase
        .from('words')
        .select('id, word, korean_meaning, part_of_speech')
      ...
      if (!words || words.length === 0) {
        return c.json({ success: true, message: 'No words need part of speech', updated: 0 });
      }

      console.log(`📝 Generating part of speech for ${words.length} words...`);

      for (const word of words) {
        if (word.part_of_speech && word.part_of_speech.trim() !== '') {
          continue; // Skip if already has part of speech
        }

        const pos = await getPartOfSpeech(word.word, word.korean_meaning || '');
        ...
        if (pos) {
          ...
          console.log(`❌ Failed to get POS for ${word.word}`);
          failedCount++;
        }
      ...
      if (!words || words.length === 0) {
        return c.json({ error: 'No words found with given IDs' }, 404);
      }

      console.log(`📝 Generating part of speech for ${words.length} specific words...`);
      ...
      return c.json({ error: 'Either wordIds or allWords must be specified' }, 400);
    }

    return c.json({
      success: true,
      message: `Part of speech generation complete`,
      updated: updatedCount,
      failed: failedCount
    });
  } catch (error) {
    console.error('Server error generating part of speech:', error);
    return c.json({ error: `Server error: ${error.message}` }, 500);
  }
});
